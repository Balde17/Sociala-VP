...
func EncodeJson1(w http.ResponseWriter, typ string, _posts interface{}, data models.Data, clients map[*websocket.Conn]string, idGroup int) error {
	jsonData, err := json.Marshal(map[string]interface{}{"type": typ, "idGroup": idGroup, "data": _posts})
	if err != nil {
		data.Error = err.Error()
		SendJSON(w, http.StatusInternalServerError, data)
		return err
...
func EncodeJson2(w http.ResponseWriter, typ string, _posts interface{}, data models.Data, clients *websocket.Conn, idGroup int, types string) error {
	jsonData, err := json.Marshal(map[string]interface{}{"type": typ, "idGroup": idGroup, "data": _posts, "types": types})
	if err != nil {
		data.Error = err.Error()
		SendJSON(w, http.StatusInternalServerError, data)
		return err
...
fmt.Println("clients",clients)
	if err := clients.WriteMessage(websocket.TextMessage, jsonData); err != nil {
		log.Printf("Erreur lors de l'envoi des posts au client: %v", err)
	}
	return nil
}
...
func ValidateStruct(model interface{}) (bool, error) {
	data := models.Data{}
	_errors := validators.Validate(model)
	...
	if len(_errors) > 0 {
		...
		data.Error = __errors
		return false, errors.New(__errors)
	}
	return true, nil
}
